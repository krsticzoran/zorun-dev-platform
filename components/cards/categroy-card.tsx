...
export function CategoryCard({
  ...
  return (
    ...
    >
      ...
      <div
        className={`pt-6 px-5 pb-4 flex flex-col gap-5 ${isFeatured ? 'xl:w-1/2 xl:gap-8' : ''}`}
      >
        <div className="flex justify-between text-custom-dark text-sm leading-5 font-semibold">
          <p className={`w-fit inline-flex items-center gap-1 py-1.5 px-3 ${readingTimeBgColor}  `}>
        ...
        <TitleTag className={`text-xl sm:text-[22px] xl:text-[26px] font-medium text-black`}>
          {post.title}
        </TitleTag>
