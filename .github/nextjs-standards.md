...
- **Naming Conventions:**
  - Use `kebab-case` for component files (e.g., `user-card.tsx`, `home-hero.tsx`).
  - Use `camelCase` for hooks (e.g., `useUser.ts`).
  - Use `kebab-case` for static assets (e.g., `logo-dark.svg`).
  - Name context providers as `XyzProvider` (e.g., `ThemeProvider`).
- **File Naming:**
  - Use `kebab-case` for all component and utility files.
  - Do **not** use barrel `index.ts` files — import components directly by file path.
  - For single-export files, use named exports.
- **Component Location:**
...
- **No barrel files:** Always import components directly, never via `index.ts` re-exports.
